...
final PVector[] originalLocations = {
...
void drawTowerSlots() {
  for (int i = 0; i < towerCount; i++) {
    ...
    if (i >= towerCount) fill(138, 166, 166);
...
void drawTowerBar() {
  ...
  text("AUTO", 514.5/3, height-272.5/3);
  text("START", 514.5/3, height-212.5/3);
  textSize(15);
  text("(space)", 514.5/3, height-152.5/3);
  textSize(18);
}
