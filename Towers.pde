...
int defdmg = 5, eightdmg = 4, slowdmg = 2, boomerangdmg = 4, sniperdmg = 10, bombdmg = 4;
int shots = 8;


boolean[] held = {false, false, false, false, false, false};
int[] towerPrice = {250, 450, 300, 450, 400, 600};
color[] towerColours = {#7b9d32, #ff86dd, #86faff, #fff41a, #43892f, #666666};

ArrayList<int[]> towerData;
int[] towerVisions = {200, 150, 250, 200, 10000, 125};
...
void drawSelectedTowers() {
  ...
  for (int towerType = 0; towerType < towerCount; towerType++) {
    ...
    if (!hasSufficientFunds(towerPrice[towerType]) && buttonHovered() == towerType) {
      ...
      drawTowerIcon(location.x, location.y, towerColours[towerType], 0);
      fill(0);
    }
