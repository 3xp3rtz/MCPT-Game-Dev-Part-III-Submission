...
int defdmg = 6, eightdmg = 4, slowdmg = 1, boomerangdmg = 4, sniperdmg = 20, bombdmg = 10;
int shots = 8;
  ...
  if (playingLevel) {
    for (int i = 0; i < towers.size(); i++) {
      ...
      if (data[cooldownRemaining] <= 0 && balloon != null) {
        ...
        if (data[projectileType] == def) {
          ...
          final int speed = 10 + data[upgrade], damage = boomerangdmg, pierce = 25, thickness = 4, maxTravelDist = 400 + (data[upgrade]-1)*25;
          PVector unitVector = PVector.div(toMouse, toMouse.mag());
          PVector velocity = PVector.mult(unitVector, speed);
          ...
          final int speed = 10, damage = bombdmg, pierce = 1, thickness = 20, maxTravelDist = 300; // slow speed, high damage, exploding shot, low range
          PVector unitVector = PVector.div(toMouse, toMouse.mag());
    ...
    for (int projectileID = 0; projectileID < projectileData.size(); projectileID++) {
      ...
      if (dead(projectileID)) {
        float[] data = projectileData.get(projectileID);
        if (data[dmgType] == bomb) createExplosion(projectileID, center.get(projectileID), data[damage] + (data[upgrade]-1)*2, (data[upgrade]-1)*0.5 + 1.2);
...
void createExplosion(float projectileID, PVector loc, float dmg, float blastRadius) {
  ...
  for (float[] balloon : balloons) {
    ...
    if (dist(balloonLoc.x, balloonLoc.y, loc.x, loc.y) <= blastRadius/2 && !balloonsHit.get((int)projectileID).contains(balloon[ID])) {
